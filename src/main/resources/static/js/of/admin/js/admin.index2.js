...
layui.use(['layim','admin.menu2','layer','admin.tab','admin.common'], function(){
	...
	function hashChanged(){
		...
		if(!$.isEmpty(hash) && hash.length>1){
			...
			//获取layId
			var dom = $('#adminLeftMenu a[menuid="'+ menuId +'"]').parent();
			if(dom.length>0){
				adminTab.add(dom);
	 ...
	 $(".menu-sub").on("click", function () {
		...
		var that = this;
		 $.each($(this).parent().siblings(), function (i, e) {
			 ...
			 $(this).find("span").addClass('layui-nav-down');          
         });
		 ...
		 $(this).children("span").toggleClass(function () {
		        if ($(this).hasClass("layui-nav-more")) {
		        	 $(this).removeClass('layui-nav-more');
		            return "layui-nav-down";
		        } else {
		        	$(this).removeClass('layui-nav-down');
		            return "layui-nav-more";
		        }
		    });
		return false;
	}) 
     $("#adminLeftMenu dl").on("click", "dd a", function () {
         $.each($(this).parent().siblings(), function (i, e) {
    ...
    $(".open").on("click",	function() {
	  ...
	  * tab菜单处理
	  */
	 $('.closeBox li dd').on("click",function(){
		   var that =  $('#adminMainTab .layui-this');
	    	var layId = $(that).attr("lay-id");
	    	$(this).removeClass('layui-this');
	    	var key = $(this).attr("lay-event");
	    	 switch (key) {
				case "close":
					if(layId==="0000"){
						break;
					...
					break;
				case "closeOther":
					$(that).parent().children("li").each(function(i,e){
